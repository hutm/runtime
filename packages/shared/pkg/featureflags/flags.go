...
var (
	...
	// Keep the upstream 50 ms default, while allowing clusters without
	// LaunchDarkly to avoid cutting off an otherwise successful first /init
	// request during snapshot resume.
	EnvdInitTimeoutMilliseconds = NewIntFlag(
		"envd-init-request-timeout-milliseconds",
		envPositiveIntOr("ENVD_INIT_REQUEST_TIMEOUT_MILLISECONDS", 50),
	)
